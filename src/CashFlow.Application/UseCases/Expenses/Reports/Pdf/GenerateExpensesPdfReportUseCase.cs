...
namespace CashFlow.Application.UseCases.Expenses.Reports.Pdf
{
    public class GenerateExpensesPdfReportUseCase : IGenerateExpensesPdfReportUseCase
    {
        ...
        private const int HEIGHT_ROW_EXPENSE_TABLE = 30;
        private readonly IExpensesReadOnlyRepository _repository;
        private readonly ILoggedUser _loggedUser;
        public GenerateExpensesPdfReportUseCase(
        ...
        public async Task<byte[]> Execute(DateOnly month)
        {
            ...
            foreach (var expense in expenses) 
            {
                ...
                AddAmoutHeader(row.Cells[3]);
                
                row = table.AddRow();
                row.Height = HEIGHT_ROW_EXPENSE_TABLE;

                row.Cells[0].AddParagraph(expense.Date.ToString("D"));
                AddSetStyleBaseForExpenseInformation(row.Cells[0]);
                row.Cells[0].Format.LeftIndent = 20;

                row.Cells[1].AddParagraph(expense.Date.ToString("t"));
                AddSetStyleBaseForExpenseInformation(row.Cells[1]);

                row.Cells[2].AddParagraph(expense.PaymentType.PaymentTypeToString());
                AddSetStyleBaseForExpenseInformation(row.Cells[2]);
        ...
        private void AddAmoutHeader(Cell cell)
        {
        ...
        private void AddSetStyleBaseForExpenseInformation (Cell cell)
        {
